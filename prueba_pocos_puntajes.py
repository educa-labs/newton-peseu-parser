...
with open('usuarios.csv', 'r') as file:
    ...
    for fila in lector_csv:
        todos_los_ruts.append((fila['rut'], fila['carrera'], fila['universidad']))


for rut in range(1000):
    rut_random = random.choice(todos_los_ruts)
    data = [
            ('tipo', 'anteriores'),
            ('rut', rut_random[0]),
            ('admision', '2013'),
    ...
    if len(puntajes_personas) > 0:
        puntajes_personas.append(rut_random[1] + ' - ' + rut_random[2])
