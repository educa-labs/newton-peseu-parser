...
pagina = requests.get('http://www.peseu.com/listas-de-seleccionados/2013/')
...
for u in ues:
    ...
    pagina_carreras = requests.get(
        'http://www.peseu.com/listas-de-seleccionados/2013/{}'.format(u))
    tree_carreras = html.fromstring(pagina_carreras.content)
    nombres_carreras = tree_carreras.xpath('//tbody/tr/td/a/text()')[1::2]
    codigos_carreras = tree_carreras.xpath('//tbody/tr/td/a/text()')[::2]

    # Guardar las carreras con sus codigos en una tupla
    universidades_con_carreras[u] = list(
        zip(codigos_carreras, nombres_carreras))
...
with open('usuarios_peseu_2013.csv', 'w', newline='') as file:
    ...
    for universidad in universidades_con_carreras:
        ...
        for carrera in universidades_con_carreras[universidad]:
            ...
            # Conseguir la pagina de las postulaciones
            pagina_postulacion = requests.get('http://www.peseu.com/listas-de-seleccionados/2013/{}/{}'.format(
                universidad, carrera[0]), headers=headers, cookies=cookies)
            ...
            personas_nombre = tree_postulacion.xpath(
                '//tbody/tr/td/text()')[2::5]
            personas_puntaje = tree_postulacion.xpath(
                '//tbody/tr/td/text()')[3::5]
            personas_situacion = tree_postulacion.xpath(
                '//tbody/tr/td/text()')[4::5]
            ...
            personas_con_carreras[key_a_guardar] = list(
                zip(personas_rut, personas_nombre, personas_puntaje, personas_situacion))
            # Iterar en cada persona que se encontró para guardar en csv
            for persona in personas_con_carreras[key_a_guardar]:
                escritor_csv.writerow(persona + (universidad, carrera[1]))
